...
class InfoPage(BoxLayout):
    ...
    cpu_use  = StringProperty("0")
    mem_use  = StringProperty("0")
    sd_use   = StringProperty("0")

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.cpu_temp = str(read_cpu_temperature())
        self.cpu_use = str(read_cpu_usage())
        self.mem_use = str(read_memory_usage())
        self.sd_use = str(read_sd_card_usage())
